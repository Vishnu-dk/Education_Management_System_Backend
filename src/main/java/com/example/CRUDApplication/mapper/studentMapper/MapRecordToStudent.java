...
public class MapRecordToStudent {
    ...
    public static Student recordToStudentMap(Record record) {
        ...
        student.setEmail(record.get("email", String.class));
        student.setPassword(record.get("password", String.class));
        Boolean active = record.get("active", Boolean.class);
        student.setActive(active != null && active);
